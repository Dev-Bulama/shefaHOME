...
<div class="space-y-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <h1 class="text-xl font-bold text-gray-800">FAQs</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-gray-800 max-w-xs">
                            <p class="truncate font-medium" title="{{ $faq->question }}">{{ Str::limit($faq->question, 80) }}</p>
                        </td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3 text-gray-500">{{ $faq->sort_order ?? 0 }}</td>
                        <td class="px-4 py-3">
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium {{ $faq->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                                {{ $faq->is_active ? 'Active' : 'Hidden' }}
                            </span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.faqs.edit', $faq) }}" class="text-blue-600 hover:text-blue-800 text-xs font-medium">Edit</a>
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="px-4 py-10 text-center text-gray-400">No FAQs found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="px-4 py-4 border-t border-gray-100">{{ $faqs->withQueryString()->links() }}</div>
        @endif
    </div>
</div>
@endsection
