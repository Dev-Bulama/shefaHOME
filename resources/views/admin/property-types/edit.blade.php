...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h2 class="text-xl font-bold text-[#0A1628]">Edit Property Type</h2>
        ...
        <form action="{{ route('admin.property-types.update', $propertyType) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div id="iconPreviewBox"
                         class="w-12 h-12 bg-[#0A1628]/5 rounded-xl flex items-center justify-center text-2xl border border-gray-200 flex-shrink-0">
                        {{ $propertyType->icon ?: '?' }}
                    </div>
                </div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          ...
                          class="w-full border border-gray-300 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#C9A84C]/40 focus:border-[#C9A84C] transition resize-none @error('description') border-red-400 @enderror">{{ old('description', $propertyType->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
