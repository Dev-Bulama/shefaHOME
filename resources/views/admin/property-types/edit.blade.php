...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h2 class="text-xl font-bold text-[#1A237E]">Edit Property Type</h2>
            <p class="text-sm text-gray-500 mt-0.5">Update <strong>{{ $propertyType->name }}</strong>.</p>
        </div>
    </div>

    {{-- Form Card --}}
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form action="{{ route('admin.property-types.update', $propertyType) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $propertyType->name) }}" required
            ...
            <div>
                <label for="slug" class="block text-sm font-semibold text-gray-700 mb-1.5">
                    Slug
                    <span class="text-xs font-normal text-gray-400 ml-1">(changing may break existing links)</span>
                </label>
                <div class="flex items-center border border-gray-300 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-[#27AE22]/40 focus-within:border-[#27AE22] transition @error('slug') border-red-400 @enderror">
                    <span class="px-3 py-2.5 text-sm text-gray-400 bg-gray-50 border-r border-gray-300 select-none">/types/</span>
                    <input type="text" id="slug" name="slug" value="{{ old('slug', $propertyType->slug) }}"
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    <input type="text" id="icon" name="icon" value="{{ old('icon', $propertyType->icon) }}"
                    ...
                    <div id="iconPreviewBox"
                         class="w-12 h-12 bg-[#1A237E]/5 rounded-xl flex items-center justify-center text-2xl border border-gray-200 flex-shrink-0">
                        {{ $propertyType->icon ?: '?' }}
                    </div>
                </div>
            ...
            <div>
                ...
                <select id="listing_type" name="listing_type"
                    ...
                    @foreach(['rent' => 'For Rent only', 'buy' => 'For Sale only', 'buy_and_rent' => 'Rent &amp; Sale', 'shortlet' => 'Short Let only'] as $val => $lbl)
                    <option value="{{ $val }}" {{ old('listing_type', $propertyType->listing_type) == $val ? 'selected' : '' }}>{!! $lbl !!}</option>
                    @endforeach
                </select>
                @error('listing_type')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          ...
                          class="w-full border border-gray-300 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#27AE22]/40 focus:border-[#27AE22] transition resize-none @error('description') border-red-400 @enderror">{{ old('description', $propertyType->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
