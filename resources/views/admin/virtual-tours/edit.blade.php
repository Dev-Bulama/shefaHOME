...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form action="{{ route('admin.virtual-tours.update', $tour) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
                ...
                <select id="property_id" name="property_id" required
                    ...
                    @foreach($properties as $property)
                        <option value="{{ $property->id }}"
                            {{ old('property_id', $tour->property_id) == $property->id ? 'selected' : '' }}>
                            {{ $property->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select id="type" name="type" required
                    ...
                    <option value="other"      {{ old('type', $tour->type) === 'other'      ? 'selected' : '' }}>Other</option>
                </select>
                @error('type')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
