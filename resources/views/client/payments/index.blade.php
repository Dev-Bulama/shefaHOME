...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <h2 class="text-[#1A237E] text-2xl font-bold font-['Playfair_Display']">Payment History</h2>
    ...
    <div class="bg-white rounded-2xl shadow-sm p-5">
        <div class="flex items-center justify-between mb-2">
            <h3 class="text-[#1A237E] font-bold text-sm font-['Playfair_Display']">Overall Portfolio Progress</h3>
            <span class="text-[#27AE22] font-bold text-sm">{{ $portfolioPct }}% paid</span>
        </div>
        <div class="w-full bg-gray-100 rounded-full h-3 mb-2">
            <div class="bg-gradient-to-r from-[#27AE22] to-[#4ADE80] h-3 rounded-full transition-all duration-700"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {{-- Total Paid --}}
        <div class="bg-white rounded-xl border-t-4 border-[#27AE22] shadow-sm p-5">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-gray-500 text-xs font-medium uppercase tracking-wide">Total Paid</p>
                    <p class="text-[#1A237E] text-2xl font-bold mt-1">₦{{ number_format($paid, 0) }}</p>
        ...
        <div class="bg-white rounded-xl border-t-4 border-red-400 shadow-sm p-5">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <p class="text-[#1A237E] text-2xl font-bold mt-1">₦{{ number_format($balance, 0) }}</p>
        ...
        <div class="bg-white rounded-xl border-t-4 border-[#1A237E] shadow-sm p-5">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-gray-500 text-xs font-medium uppercase tracking-wide">Portfolio Value</p>
                    <p class="text-[#1A237E] text-2xl font-bold mt-1">₦{{ number_format($totalPortfolio, 0) }}</p>
                    <p class="text-gray-400 text-xs mt-1">Total investment value</p>
                </div>
                <div class="w-10 h-10 bg-[#1A237E]/10 rounded-lg flex items-center justify-center flex-shrink-0">
    ...
    <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h3 class="text-[#1A237E] font-bold text-base font-['Playfair_Display']">All Payments</h3>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-5 py-4 max-w-[160px]">
                            <span class="text-[#1A237E] font-semibold text-xs block truncate">
                        ...
                        <td class="px-5 py-4">
                            <span class="text-[#1A237E] font-semibold text-xs">₦{{ number_format($payment->total_price, 0) }}</span>
                        </td>
                        <td class="px-5 py-4">
                            <span class="text-[#27AE22] font-semibold text-xs">₦{{ number_format($payment->amount_paid, 0) }}</span>
                        </td>
                        <td class="px-5 py-4">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('client.payments.receipt', $payment->id) }}"
                                   class="inline-flex items-center gap-1 text-[#1A237E] hover:text-[#27AE22] text-xs font-semibold transition-colors">
                                ...
                                <a href="{{ route('client.payments.make', $payment->id) }}"
                                   class="inline-flex items-center gap-1 bg-[#27AE22] hover:bg-[#b8963e] text-white text-xs font-semibold px-2.5 py-1 rounded-lg transition-colors">
                                    Pay
                                </a>
                                @endif
                            </div>
