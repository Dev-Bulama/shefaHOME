...
<div class="print:hidden mb-6 flex items-center justify-between">
    <a href="{{ route('client.payments.index') }}"
       class="inline-flex items-center gap-2 text-sm text-gray-600 hover:text-[#0A1628] font-semibold transition-colors">
    ...
    <button onclick="window.print()"
            class="inline-flex items-center gap-2 bg-[#C9A84C] hover:bg-[#b8963e] text-white font-semibold px-5 py-2.5 rounded-xl transition-colors text-sm shadow-md shadow-[#C9A84C]/20">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="receipt-paper max-w-2xl mx-auto bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
    ...
    {{-- Letterhead --}}
    <div class="bg-[#0A1628] px-8 py-8 relative overflow-hidden">
        ...
        <div class="relative flex items-start justify-between">
            <div>
                <div class="flex items-center gap-3 mb-3">
                    <div class="w-10 h-10 bg-[#C9A84C] rounded-xl flex items-center justify-center">
                        <svg class="w-6 h-6 text-[#0A1628]" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div>
                        ...
                        <p class="text-[#C9A84C] text-xs tracking-wide">Premium Real Estate &mdash; Client Portal</p>
                    </div>
                </div>
            ...
            <div class="text-right">
                <div class="bg-white/10 border border-[#C9A84C]/30 rounded-xl px-4 py-3">
                    <p class="text-[#C9A84C] text-xs uppercase tracking-widest mb-1">Official Receipt</p>
    ...
    <div class="px-8 py-6 space-y-6">
        ...
        <div class="flex justify-between items-start flex-wrap gap-4">
            <div>
                ...
                <p class="text-[#0A1628] font-bold text-sm">
            ...
            <div class="text-right">
                ...
                <p class="text-[#0A1628] font-bold text-sm">{{ $payment->payment_plan ?? 'N/A' }}</p>
            </div>
        </div>
            ...
            <div class="bg-gray-50 rounded-xl p-4">
                ...
                <div class="space-y-1.5">
                    <div>
                        ...
                        <p class="text-[#0A1628] font-semibold text-sm">{{ auth()->user()->name }}</p>
                    </div>
                    <div>
                        <span class="text-gray-500 text-xs">Client ID</span>
                        <p class="text-[#C9A84C] font-bold text-sm font-mono">
                            CLT-{{ str_pad(auth()->user()->id, 5, '0', STR_PAD_LEFT) }}
                        </p>
                    </div>
                    <div>
                        <span class="text-gray-500 text-xs">Email</span>
                        <p class="text-[#0A1628] text-sm">{{ auth()->user()->email }}</p>
                    </div>
                    @if(auth()->user()->phone)
                    <div>
                        <span class="text-gray-500 text-xs">Phone</span>
                        <p class="text-[#0A1628] text-sm">{{ auth()->user()->phone }}</p>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-gray-50 rounded-xl p-4">
                ...
                <div class="space-y-1.5">
                    <div>
                        ...
                        <p class="text-[#0A1628] font-semibold text-sm">{{ $property->name ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <span class="text-gray-500 text-xs">Location</span>
                        <p class="text-[#0A1628] text-sm">
                    ...
                    <div>
                        ...
                        <p class="text-[#0A1628] text-sm">{{ $property->plot_size }}</p>
                    </div>
                    @endif
                    <div>
                        <span class="text-gray-500 text-xs">Payment Plan</span>
                        <p class="text-[#0A1628] text-sm">{{ $payment->payment_plan ?? 'N/A' }}</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-[#0A1628] font-bold text-right">₦{{ number_format($totalPrice, 0) }}</td>
                        </tr>
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 text-gray-700">
                            ...
                            <td class="px-4 py-3 text-[#C9A84C] font-bold text-right">(₦{{ number_format($amountPaid, 0) }})</td>
                        </tr>
                        <tr class="bg-gray-50">
                            <td class="px-4 py-3">
                                <p class="font-bold text-[#0A1628]">Outstanding Balance</p>
                            </td>
        ...
        <div class="border-t border-gray-100 pt-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            ...
            <div class="text-right">
                <p class="text-[#C9A84C] font-bold text-sm font-['Playfair_Display']">SHEFAHOMES Ltd</p>
                <p class="text-gray-400 text-xs">Authorized Signature</p>
                <div class="mt-1 w-24 border-b-2 border-[#0A1628]"></div>
            </div>
        </div>
    </div>
</div>
...
<div class="print:hidden mt-6 flex flex-col sm:flex-row items-center justify-center gap-3">
    <button onclick="window.print()"
            class="inline-flex items-center gap-2 bg-[#C9A84C] hover:bg-[#b8963e] text-white font-semibold px-6 py-3 rounded-xl transition-colors text-sm">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/>
        </svg>
        Download PDF
    ...
    <a href="{{ route('client.payments.index') }}"
       class="inline-flex items-center gap-2 border border-gray-300 hover:border-[#0A1628] text-gray-600 hover:text-[#0A1628] font-semibold px-6 py-3 rounded-xl transition-colors text-sm">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
        </svg>
        Back to Payments
