...
<div class="max-w-4xl mx-auto space-y-6" x-data="{ activeTab: 'personal' }">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-[#1A237E] font-display">My Profile</h1>
    ...
    <div class="flex gap-1 bg-gray-100 rounded-xl p-1 w-fit">
        <button @click="activeTab='personal'" :class="activeTab==='personal' ? 'bg-white text-[#1A237E] shadow-sm' : 'text-gray-500 hover:text-gray-700'" class="px-5 py-2.5 rounded-lg text-sm font-semibold transition-all">Personal Info</button>
        <button @click="activeTab='security'" :class="activeTab==='security' ? 'bg-white text-[#1A237E] shadow-sm' : 'text-gray-500 hover:text-gray-700'" class="px-5 py-2.5 rounded-lg text-sm font-semibold transition-all">Security</button>
    </div>
    ...
    <form action="{{ route('client.profile.update') }}" method="POST" enctype="multipart/form-data">
        ...
        <div x-show="activeTab==='personal'" class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <h2 class="font-bold text-[#1A237E] mb-5 text-lg">Profile Photo</h2>
                <div class="flex items-center gap-6">
                    <img id="avatarPreview" src="{{ $user->avatar_url }}" alt="{{ $user->name }}"
                         class="w-24 h-24 rounded-full object-cover ring-4 ring-[#27AE22]/20">
                    <div>
                        <label for="avatar" class="cursor-pointer inline-flex items-center gap-2 bg-[#1A237E] text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[#0D1566] transition-colors">
            ...
            {{-- Personal Details --}}
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <h2 class="font-bold text-[#1A237E] mb-5 text-lg">Personal Details</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Full Name *</label>
                        <input type="text" name="name" value="{{ old('name', $user->name) }}" required
                               class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#27AE22] focus:border-transparent outline-none transition-all text-sm">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="state" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#27AE22] focus:border-transparent outline-none transition-all text-sm">
            ...
            {{-- Next of Kin --}}
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <h2 class="font-bold text-[#1A237E] mb-5 text-lg">Next of Kin</h2>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ...
                    <div>
                        ...
                        <select name="nok_relationship" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#27AE22] focus:border-transparent outline-none transition-all text-sm">
            ...
            <div class="flex justify-end">
                <button type="submit" class="bg-[#27AE22] text-[#1A237E] font-bold px-8 py-3 rounded-xl hover:bg-[#4ADE80] transition-all shadow-md hover:shadow-lg">
                    Save Changes
                </button>
            </div>
        </div>
        ...
        <div x-show="activeTab==='security'" class="space-y-6" style="display:none">
            ...
            {{-- Change Password --}}
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <h2 class="font-bold text-[#1A237E] mb-5 text-lg">Change Password</h2>
            ...
            <div class="flex justify-end">
                <button type="submit" class="bg-[#1A237E] text-white font-bold px-8 py-3 rounded-xl hover:bg-[#0D1566] transition-all shadow-md hover:shadow-lg">
                    Update Password
                </button>
            </div>
        </div>
