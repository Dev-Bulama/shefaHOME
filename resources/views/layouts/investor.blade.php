...
.tier-badge-gold { background: linear-gradient(135deg, #27AE22, #4ADE80); color: #1A237E; }
...
<div class="flex h-screen overflow-hidden">
    ...
    <aside :class="sidebarOpen ? 'w-64' : 'w-16'"
        ...
        <div class="flex items-center gap-3 px-4 py-5 border-b border-white/10 min-h-[72px]">
            ...
            <div x-show="sidebarOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
                ...
                <span class="text-[#27AE22] text-xs">Investor Portal</span>
            </div>
        </div>
        ...
        <div x-show="sidebarOpen" class="mx-3 mt-4 mb-2 p-3 rounded-xl bg-white/5 border border-white/10">
            <div class="flex items-center gap-2 mb-2">
                <div class="w-9 h-9 rounded-full overflow-hidden ring-2 ring-[#27AE22]/40 flex-shrink-0">
                    <img src="{{ auth()->user()->avatar_url ?? 'https://ui-avatars.com/api/?name='.urlencode(auth()->user()->name ?? 'Investor').'&background=1A237E&color=27AE22' }}"
                         alt="{{ auth()->user()->name }}"
        ...
        <div x-show="sidebarOpen" class="mx-3 mb-3 p-3 rounded-xl bg-[#27AE22]/10 border border-[#27AE22]/20">
            <p class="text-[#27AE22] text-xs font-semibold mb-2 uppercase tracking-wider">Quick Stats</p>
            <div class="space-y-1.5">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <span class="text-[#27AE22] text-xs font-semibold">{{ auth()->user()->investor->total_roi_formatted ?? '—' }}</span>
                </div>
            </div>
        </div>
    ...
    <aside x-show="mobileOpen"
        ...
        <nav class="flex-1 px-2 py-4 space-y-0.5">
            ...
            <a href="{{ route('investor.profile') }}" class="sidebar-link">My Profile</a>
        </nav>
        <div class="px-2 py-4 border-t border-white/10">
            <form action="{{ route('logout') }}" method="POST">
                @csrf
    ...
    <div class="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <header class="bg-white shadow-sm px-4 lg:px-6 py-3 flex items-center justify-between flex-shrink-0">
            ...
            <div class="flex items-center gap-3" x-data="{ dropOpen: false }">
                {{-- Notifications --}}
                <button class="relative text-gray-500 hover:text-[#1A237E] p-1 rounded transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="relative">
                    ...
                    <div x-show="dropOpen"
                        ...
                        <a href="{{ route('investor.profile') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors">My Profile</a>
                        <form action="{{ route('logout') }}" method="POST">
                            @csrf
