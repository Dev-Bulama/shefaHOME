...
<section class="bg-[#0A1628] py-20 relative overflow-hidden">
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <nav class="flex items-center justify-center gap-2 text-sm text-gray-400 mb-6 flex-wrap">
            ...
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            <span class="text-[#C9A84C]">{{ $category->name ?? 'Category' }}</span>
        </nav>
        <span class="inline-block bg-[#C9A84C] text-[#0A1628] text-xs font-bold px-4 py-2 rounded-full mb-4">Category</span>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-2 overflow-x-auto py-4">
            ...
            <a href="{{ route('blog.category', $cat->slug) }}"
               class="flex-shrink-0 px-5 py-2 rounded-full text-sm font-semibold transition-all {{ isset($category) && $category->slug === $cat->slug ? 'bg-[#C9A84C] text-[#0A1628]' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
                {{ $cat->name }}
            </a>
            @endforeach
            @endif
...
<section class="py-16 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center py-20">
            ...
            <h3 class="font-bold text-[#0A1628] text-xl mb-2">No Posts in This Category</h3>
            <p class="text-gray-400 mb-6">Check back soon, or browse all articles.</p>
            <a href="{{ route('blog.index') }}" class="bg-[#C9A84C] text-[#0A1628] font-bold px-8 py-3 rounded-full hover:bg-[#E8C97A] transition-all">
                Browse All Posts
            </a>
        </div>
        @endif
