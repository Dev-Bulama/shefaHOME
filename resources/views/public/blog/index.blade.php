...
<section class="relative h-[70vh] min-h-[500px] bg-[#0A1628] overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
            <div class="max-w-2xl">
                ...
                <a href="{{ route('blog.show', $featuredPost->slug) }}"
                   class="inline-flex items-center gap-2 bg-[#C9A84C] text-[#0A1628] font-bold px-8 py-4 rounded-full hover:bg-[#E8C97A] transition-all hover:scale-105">
                    Read Article
...
<section class="bg-[#0A1628] py-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <span class="inline-block text-[#C9A84C] font-semibold text-sm tracking-widest uppercase mb-3">Real Estate Insights</span>
        <h1 class="font-display text-5xl font-bold text-white mb-4">Our <span class="text-[#C9A84C]">Blog</span></h1>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-2 overflow-x-auto py-4 scrollbar-hide">
            <a href="{{ route('blog.index') }}"
               class="flex-shrink-0 px-5 py-2 rounded-full text-sm font-semibold transition-all {{ !request('category') ? 'bg-[#0A1628] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
                All Posts
            </a>
            @if(isset($categories))
            @foreach($categories as $cat)
            <a href="{{ route('blog.category', $cat->slug) }}"
               class="flex-shrink-0 px-5 py-2 rounded-full text-sm font-semibold transition-all {{ request()->routeIs('blog.category') && request()->route('category') == $cat->slug ? 'bg-[#C9A84C] text-[#0A1628]' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
                {{ $cat->name }}
...
<section class="py-16 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <article class="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:shadow-[#0A1628]/10 border border-gray-100 transition-all duration-500 hover:-translate-y-1 flex flex-col"
                ...
                <div class="p-6 flex flex-col flex-1">
                    ...
                    <h2 class="font-display font-bold text-[#0A1628] text-lg mb-3 leading-snug group-hover:text-[#C9A84C] transition-colors line-clamp-2">
        ...
        <div class="text-center py-20">
            ...
            <h3 class="font-bold text-[#0A1628] text-xl mb-2">No Posts Yet</h3>
