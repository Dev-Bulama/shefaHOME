...
<section class="bg-[#0A1628] py-24 relative overflow-hidden">
    <div class="absolute inset-0 opacity-10">
        <div class="absolute -top-10 -right-10 w-80 h-80 bg-[#C9A84C] rounded-full"></div>
        <div class="absolute -bottom-10 -left-10 w-60 h-60 bg-[#C9A84C] rounded-full"></div>
    </div>
    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <span class="inline-block text-[#C9A84C] font-semibold text-sm tracking-widest uppercase mb-3">Join Our Team</span>
        <h1 class="font-display text-5xl lg:text-6xl font-bold text-white mb-5">Build Your <span class="text-[#C9A84C]">Career</span> with Us</h1>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <p class="text-gray-500 text-sm">
                <span class="font-semibold text-[#0A1628]">{{ isset($jobs) ? $jobs->count() : 0 }}</span> open positions
            </p>
        </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all"
                ...
                <button @click="open = !open"
                    ...
                    <div class="flex-1">
                        <div class="flex flex-wrap items-center gap-2 mb-2">
                            <span class="bg-[#C9A84C]/15 text-[#b8943d] text-xs font-bold px-3 py-1 rounded-full">
                        ...
                        <h3 class="font-display font-bold text-[#0A1628] text-lg leading-snug">{{ $job->title }}</h3>
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="border-t border-gray-100">
                    <div class="p-6">
                        ...
                        <div class="mb-5">
                            <h4 class="font-semibold text-[#0A1628] text-sm mb-3">Requirements</h4>
                            <ul class="space-y-2">
                                @foreach($job->requirements as $req)
                                <li class="flex items-start gap-2 text-gray-500 text-sm">
                                    <svg class="w-4 h-4 text-[#C9A84C] mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                    {{ $req }}
                                </li>
                                @endforeach
                            </ul>
        ...
        <div class="text-center py-20 bg-white rounded-2xl border border-gray-100">
            ...
            <h3 class="font-bold text-[#0A1628] text-xl mb-2">No Open Positions</h3>
            ...
            <a href="{{ route('contact') }}" class="bg-[#C9A84C] text-[#0A1628] font-bold px-8 py-3 rounded-full hover:bg-[#E8C97A] transition-all">
...
<section class="py-20 bg-[#0A1628]">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <a href="{{ route('contact') }}" class="bg-[#C9A84C] hover:bg-[#E8C97A] text-[#0A1628] font-bold px-10 py-4 rounded-full transition-all hover:scale-105">
            Express Interest
        </a>
    </div>
</section>
