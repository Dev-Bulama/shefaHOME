...
<section class="bg-[#1A237E] py-20 relative overflow-hidden">
    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <nav class="flex items-center gap-2 text-sm text-gray-400 mb-6 flex-wrap">
            ...
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            <span class="text-[#27AE22]">{{ $job->title ?? 'Job' }}</span>
        </nav>
        <div class="flex flex-col lg:flex-row gap-8 items-start">
            <div class="flex-1">
                <div class="flex flex-wrap gap-2 mb-4">
                    <span class="bg-[#27AE22]/20 text-[#27AE22] text-xs font-bold px-3 py-1.5 rounded-full">{{ $job->department ?? 'General' }}</span>
            ...
            <div class="flex-shrink-0">
                <a href="{{ route('careers.show', $job->slug ?? $job->id) }}"
                   class="block bg-[#27AE22] hover:bg-[#4ADE80] text-[#1A237E] font-bold px-10 py-4 rounded-full transition-all hover:scale-105 text-center">
                    Apply Now
                </a>
            </div>
        </div>
...
<section class="py-16 bg-gray-50">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 lg:p-10 space-y-10">
            ...
            @if($job->description)
            <div>
                <h2 class="font-display text-2xl font-bold text-[#1A237E] mb-5 pb-3 border-b border-gray-100">About This Role</h2>
            ...
            <div>
                <h2 class="font-display text-2xl font-bold text-[#1A237E] mb-5 pb-3 border-b border-gray-100">Key Responsibilities</h2>
                <ul class="space-y-3">
                    @foreach($job->responsibilities as $res)
                    <li class="flex items-start gap-3 text-gray-600 text-sm">
                        <div class="w-5 h-5 rounded-full bg-[#27AE22]/15 flex items-center justify-center flex-shrink-0 mt-0.5">
                            <svg class="w-3 h-3 text-[#27AE22]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg>
                        </div>
                        {{ $res }}
                    </li>
                    @endforeach
            ...
            <div>
                <h2 class="font-display text-2xl font-bold text-[#1A237E] mb-5 pb-3 border-b border-gray-100">Requirements</h2>
                <ul class="space-y-3">
                    @foreach($job->requirements as $req)
                    <li class="flex items-start gap-3 text-gray-600 text-sm">
                        <div class="w-5 h-5 rounded-full bg-[#1A237E]/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                            <svg class="w-3 h-3 text-[#1A237E]" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
                        </div>
                        {{ $req }}
                    </li>
                    @endforeach
            ...
            <div>
                <h2 class="font-display text-2xl font-bold text-[#1A237E] mb-5 pb-3 border-b border-gray-100">What We Offer</h2>
            ...
            {{-- Apply CTA --}}
            <div class="bg-gradient-to-r from-[#1A237E] to-[#0D1566] rounded-2xl p-8 text-center">
                ...
                <a href="{{ route('careers.show', $job->slug ?? $job->id) }}"
                   class="inline-block bg-[#27AE22] hover:bg-[#4ADE80] text-[#1A237E] font-bold px-10 py-4 rounded-full transition-all hover:scale-105">
