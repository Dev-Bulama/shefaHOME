...
<section class="py-20 bg-gray-50 overflow-hidden">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-16" data-reveal>
            <span class="inline-block text-[#C9A84C] font-semibold text-sm tracking-widest uppercase mb-3">Simple Process</span>
            <h2 class="font-display text-4xl md:text-5xl font-bold text-[#0A1628] mb-4">
                How It <span class="text-[#C9A84C]">Works</span>
            </h2>
        ...
        <div class="hidden lg:grid lg:grid-cols-4 gap-0 relative">
            {{-- Connecting line --}}
            <div class="absolute top-16 left-[12.5%] right-[12.5%] h-0.5 bg-gradient-to-r from-[#C9A84C]/20 via-[#C9A84C] to-[#C9A84C]/20" style="z-index:0"></div>

            @foreach($steps as $i => $step)
            <div class="relative flex flex-col items-center text-center px-6" data-reveal style="transition-delay: {{ $i * 150 }}ms">
                {{-- Number circle --}}
                <div class="relative z-10 w-16 h-16 rounded-full bg-[#C9A84C] text-[#0A1628] font-display font-black text-lg flex items-center justify-center shadow-lg shadow-[#C9A84C]/30 mb-6 ring-4 ring-white">
                    {{ $step['number'] }}
                </div>
                ...
                <h3 class="font-display font-bold text-[#0A1628] text-lg mb-3">{{ $step['title'] }}</h3>
        ...
        <div class="lg:hidden space-y-0">
            ...
            <div class="flex gap-6 relative" data-reveal style="transition-delay: {{ $i * 100 }}ms">
                ...
                <div class="flex flex-col items-center flex-shrink-0">
                    <div class="w-12 h-12 rounded-full bg-[#C9A84C] text-[#0A1628] font-display font-black text-base flex items-center justify-center shadow-md z-10">
                        {{ $step['number'] }}
                    </div>
                    @if(!$loop->last)
                    <div class="w-0.5 flex-1 bg-[#C9A84C]/30 my-2 min-h-[40px]"></div>
                    @endif
                </div>
                ...
                <div class="pb-8 flex-1">
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                        <h3 class="font-display font-bold text-[#0A1628] text-lg mb-2">{{ $step['title'] }}</h3>
                        <p class="text-gray-500 text-sm leading-relaxed">{{ $step['desc'] }}</p>
                    </div>
                </div>
            </div>
        ...
        <div class="text-center mt-14" data-reveal>
            <a href="{{ route('properties.index') }}"
               class="inline-flex items-center gap-2 bg-[#0A1628] hover:bg-[#C9A84C] text-white hover:text-[#0A1628] font-bold px-10 py-4 rounded-full transition-all hover:scale-105 shadow-lg">
                Get Started Today
