...
@php
    ...
    // Plain-text excerpt from description (~220 chars, 2-3 sentences)
    ...
    // Image: admin-set via Pages → Home, or a seed-based placeholder
    $imageUrl  = !empty($section['image'])
        ? (str_starts_with($section['image'], 'http') || str_starts_with($section['image'], '//')
            ? $section['image']
            : asset('uploads/'.$section['image']))
...
<div class="group bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-xl transition-all duration-500" data-reveal>
    <div class="flex flex-col {{ $isEven ? 'lg:flex-row' : 'lg:flex-row-reverse' }}">
        ...
        {{-- ── Image panel ─────────────────────────────────────────── --}}
        <div class="relative overflow-hidden lg:w-[52%] aspect-[4/3] lg:aspect-auto lg:min-h-[460px]">
            <img src="{{ $imageUrl }}"
                 alt="{{ $section['title'] }}"
            ...
            {{-- Directional gradient so image bleeds into content --}}
            <div class="absolute inset-0 bg-gradient-to-{{ $isEven ? 'r' : 'l' }} from-transparent via-transparent to-white/30 pointer-events-none"></div>

            {{-- Category pill badge --}}
            <div class="absolute top-5 {{ $isEven ? 'left-5' : 'right-5' }}">
        ...
        {{-- ── Content panel ───────────────────────────────────────── --}}
        <div class="lg:w-[48%] flex flex-col justify-center px-8 py-10 lg:px-14 lg:py-14">
            ...
            @if($excerpt)
            <p class="w-full text-gray-500 text-lg leading-relaxed mb-8">
                {{ $excerpt }}
            </p>
            @endif
