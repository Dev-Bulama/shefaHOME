...
<section class="py-20 bg-[#0A1628] relative overflow-hidden">
    ...
    <div class="absolute inset-0 opacity-5">
        <div class="absolute top-0 left-0 w-72 h-72 bg-[#C9A84C] rounded-full -translate-x-1/2 -translate-y-1/2"></div>
        <div class="absolute bottom-0 right-0 w-96 h-96 bg-[#C9A84C] rounded-full translate-x-1/3 translate-y-1/3"></div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div class="text-center mb-14">
            <span class="inline-block text-[#C9A84C] font-semibold text-sm tracking-widest uppercase mb-3">Our Track Record</span>
            <h2 class="font-display text-4xl md:text-5xl font-bold text-white mb-4">
                Numbers That <span class="text-[#C9A84C]">Speak</span>
            </h2>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12"
            ...
            <div class="text-center group" data-reveal style="transition-delay: {{ $index * 150 }}ms">
                {{-- Icon --}}
                <div class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-white/10 group-hover:bg-[#C9A84C]/20 border border-white/10 group-hover:border-[#C9A84C]/30 transition-all duration-300 mb-6 mx-auto">
                ...
                {{-- Counter --}}
                <div class="mb-2">
                    <span class="font-display text-5xl lg:text-6xl font-bold text-[#C9A84C]"
                          data-count="{{ is_array($stat) ? ($stat['value'] ?? 0) : ($stat->value ?? 0) }}">0</span>
                    <span class="font-display text-4xl font-bold text-[#C9A84C]">{{ is_array($stat) ? ($stat['suffix'] ?? '+') : ($stat->suffix ?? '+') }}</span>
                </div>
                ...
                {{-- Decorative line --}}
                <div class="mt-4 h-0.5 w-12 bg-[#C9A84C]/30 mx-auto group-hover:w-24 group-hover:bg-[#C9A84C]/60 transition-all duration-500"></div>
            </div>
            @endforeach
        </div>
        ...
        <div class="text-center mt-16 pt-12 border-t border-white/10">
            ...
            <a href="{{ route('properties.index') }}"
               class="inline-flex items-center gap-2 bg-[#C9A84C] hover:bg-[#E8C97A] text-[#0A1628] font-bold px-10 py-4 rounded-full transition-all hover:scale-105 hover:shadow-xl hover:shadow-[#C9A84C]/30">
                Start Your Journey
