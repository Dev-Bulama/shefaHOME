...
<section class="py-20 bg-[#1A237E] relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div class="text-center mb-14" data-reveal>
            <span class="inline-block text-[#27AE22] font-semibold text-sm tracking-widest uppercase mb-3">Client Stories</span>
            <h2 class="font-display text-4xl md:text-5xl font-bold text-white mb-4">
                What Our <span class="text-[#27AE22]">Clients</span> Say
            </h2>
        ...
        <div x-data="{
            ...
            <div class="overflow-hidden">
                <div class="flex transition-transform duration-700 ease-in-out gap-6"
                    ...
                    <div class="min-w-[calc(100%-2rem)] sm:min-w-[calc(50%-12px)] lg:min-w-[calc(33.333%-16px)] bg-white/10 backdrop-blur-sm border border-white/10 rounded-2xl p-7 flex flex-col hover:bg-white/15 transition-colors duration-300 group">
                        ...
                        {{-- Quote icon --}}
                        <div class="mb-4">
                            <svg class="w-10 h-10 text-[#27AE22]/40" fill="currentColor" viewBox="0 0 32 32">
                        ...
                        <div class="flex gap-1 mb-5">
                            @for($s = 1; $s <= 5; $s++)
                            <svg class="w-4 h-4 {{ $s <= ($t->rating ?? 5) ? 'text-[#27AE22]' : 'text-white/20' }}" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <div class="flex items-center gap-4 border-t border-white/10 pt-5">
                            ...
                            <img src="{{ $t->avatar_url }}" alt="{{ $t->client_name }}"
                                 class="w-12 h-12 rounded-full object-cover border-2 border-[#27AE22]/50">
                            @else
                            <div class="w-12 h-12 rounded-full bg-gradient-to-br from-[#27AE22] to-[#1D9418] flex items-center justify-center text-[#1A237E] font-bold text-lg">
                            ...
                            <div>
                                ...
                                <p class="text-[#27AE22] text-xs font-medium">Bought: {{ $t->property_bought }}</p>
                                @endif
                                @if($t->location)
            ...
            <div class="flex items-center justify-center gap-4 mt-10">
                <button @click="prev()"
                        class="w-11 h-11 rounded-full border border-white/20 flex items-center justify-center text-white hover:bg-[#27AE22] hover:border-[#27AE22] hover:text-[#1A237E] transition-all">
                ...
                <div class="flex gap-2">
                    <template x-for="i in Math.max(1, total - 2)" :key="i">
                        <button @click="current = i - 1"
                                :class="current === i - 1 ? 'bg-[#27AE22] w-6' : 'bg-white/20 w-2'"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-white/10 border border-white/10 rounded-2xl p-7 flex flex-col">
                <svg class="w-8 h-8 text-[#27AE22]/40 mb-4" fill="currentColor" viewBox="0 0 32 32">
                    <path d="M10 8C6.686 8 4 10.686 4 14v10h10V14H7c0-1.654 1.346-3 3-3V8zm18 0c-3.314 0-6 2.686-6 6v10h10V14h-7c0-1.654 1.346-3 3-3V8z"/>
                </svg>
                <p class="text-gray-300 italic text-sm leading-relaxed mb-6 flex-1">"{{ $pt['text'] }}"</p>
                <div class="flex gap-1 mb-4">
                    @for($s=0;$s<5;$s++)
                    <svg class="w-4 h-4 text-[#27AE22]" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                    @endfor
                </div>
                <div class="flex items-center gap-3 border-t border-white/10 pt-4">
                    <div class="w-10 h-10 rounded-full bg-gradient-to-br from-[#27AE22] to-[#1D9418] flex items-center justify-center text-[#1A237E] font-bold">
                        {{ strtoupper(substr($pt['name'], 0, 1)) }}
                    </div>
                    <div>
                        <p class="font-bold text-white text-sm">{{ $pt['name'] }}</p>
                        <p class="text-[#27AE22] text-xs">{{ $pt['property'] }}</p>
                    </div>
                </div>
            </div>
            @endforeach
        ...
        <div class="text-center mt-12">
            <a href="{{ route('testimonials') }}"
               class="inline-flex items-center gap-2 border-2 border-[#27AE22] text-[#27AE22] hover:bg-[#27AE22] hover:text-[#1A237E] font-bold px-8 py-4 rounded-full transition-all">
                Read More Stories
