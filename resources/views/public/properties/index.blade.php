...
<section class="relative bg-[#0A1628] py-20 overflow-hidden">
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <span class="inline-block text-[#C9A84C] font-semibold text-sm tracking-widest uppercase mb-3">Our Portfolio</span>
        <h1 class="font-display text-4xl sm:text-5xl lg:text-6xl font-bold text-white mb-4">
            Premium <span class="text-[#C9A84C]">Properties</span>
        </h1>
        ...
        <nav class="mt-6 flex items-center justify-center gap-2 text-sm text-gray-400">
            <a href="{{ route('home') }}" class="hover:text-[#C9A84C] transition-colors">Home</a>
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            <span class="text-[#C9A84C]">Properties</span>
        </nav>
    </div>
</section>
        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <aside class="lg:w-72 flex-shrink-0" x-data="{ open: false }">
                ...
                <button @click="open = !open"
                        class="lg:hidden w-full flex items-center justify-between bg-white border border-gray-200 rounded-2xl px-6 py-4 font-semibold text-[#0A1628] mb-4 shadow-sm">
                ...
                <div :class="open ? 'block' : 'hidden lg:block'"
                    ...
                    <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
                        <h3 class="font-bold text-[#0A1628]">Filters</h3>
                        <a href="{{ route('properties.index') }}" class="text-xs text-[#C9A84C] hover:underline font-medium">Clear All</a>
                    </div>
                    ...
                    <form method="GET" action="{{ route('properties.index') }}" id="filter-form" class="divide-y divide-gray-100">
                        ...
                        {{-- State --}}
                        <div class="px-6 py-5">
                            <h4 class="font-semibold text-[#0A1628] text-sm mb-4 uppercase tracking-wider">Location</h4>
                            <div class="space-y-2 max-h-48 overflow-y-auto pr-1">
                                ...
                                <label class="flex items-center gap-3 cursor-pointer group">
                                    <input type="checkbox" name="state[]" value="{{ $s }}"
                                           ...
                                           class="w-4 h-4 text-[#C9A84C] border-gray-300 rounded focus:ring-[#C9A84C]"
                                           onchange="document.getElementById('filter-form').submit()">
                                    <span class="text-gray-600 text-sm group-hover:text-[#0A1628] transition-colors">{{ $s }}</span>
                                </label>
                                @endforeach
                            </div>
                        </div>
                        ...
                        {{-- Property Type --}}
                        <div class="px-6 py-5">
                            <h4 class="font-semibold text-[#0A1628] text-sm mb-4 uppercase tracking-wider">Property Type</h4>
                            <div class="space-y-2">
                                <label class="flex items-center gap-3 cursor-pointer">
                                    <input type="radio" name="type" value="" {{ !request('type') ? 'checked' : '' }} class="w-4 h-4 text-[#C9A84C] border-gray-300 focus:ring-[#C9A84C]" onchange="this.form.submit()">
                                ...
                                <label class="flex items-center gap-3 cursor-pointer">
                                    <input type="radio" name="type" value="{{ $pt->id }}"
                                           ...
                                           class="w-4 h-4 text-[#C9A84C] border-gray-300 focus:ring-[#C9A84C]"
                        ...
                        <div class="px-6 py-5" x-data="{ min: '{{ request('min_price', '') }}', max: '{{ request('max_price', '') }}' }">
                            <h4 class="font-semibold text-[#0A1628] text-sm mb-4 uppercase tracking-wider">Price Range</h4>
                            <div class="space-y-3">
                                <div>
                                    <label class="text-xs text-gray-400 mb-1 block">Min Price (₦)</label>
                                    <input type="number" name="min_price" x-model="min"
                                           placeholder="0"
                                           class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#C9A84C] text-[#0A1628]">
                                </div>
                                <div>
                                ...
                                <button type="submit" class="w-full bg-[#0A1628] text-white text-sm font-semibold py-2.5 rounded-xl hover:bg-[#C9A84C] hover:text-[#0A1628] transition-all">Apply Price</button>
                            </div>
                        </div>

                        {{-- Status --}}
                        <div class="px-6 py-5">
                            <h4 class="font-semibold text-[#0A1628] text-sm mb-4 uppercase tracking-wider">Status</h4>
                            <div class="space-y-2">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 bg-white rounded-2xl px-6 py-4 shadow-sm border border-gray-100">
                    <p class="text-gray-500 text-sm">
                        Showing
                        <span class="font-semibold text-[#0A1628]">{{ $properties->firstItem() ?? 0 }}</span>
                        –
                        <span class="font-semibold text-[#0A1628]">{{ $properties->lastItem() ?? 0 }}</span>
                        of
                        <span class="font-semibold text-[#0A1628]">{{ $properties->total() }}</span>
                        properties
                    </p>
                    <div class="flex items-center gap-3">
                        ...
                        <select onchange="window.location.href='?{{ http_build_query(array_merge(request()->except(['sort', 'page']), [])) }}&sort=' + this.value"
                                class="border border-gray-200 rounded-xl px-4 py-2 text-sm text-[#0A1628] focus:outline-none focus:ring-2 focus:ring-[#C9A84C] bg-gray-50">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                    ...
                    <article class="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:shadow-[#0A1628]/10 border border-gray-100 transition-all duration-500 hover:-translate-y-1 flex flex-col">
                        ...
                        <div class="relative overflow-hidden h-56 flex-shrink-0">
                            ...
                            <div class="absolute inset-0 bg-gradient-to-t from-[#0A1628]/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                            ...
                            <span class="absolute top-3 right-3 bg-[#C9A84C] text-[#0A1628] text-xs font-bold px-3 py-1 rounded-full shadow">★ Featured</span>
                            @endif
                        </div>
                        ...
                        <div class="p-5 flex flex-col flex-1">
                            <div class="flex items-center gap-2 mb-2">
                                <span class="text-[#C9A84C] text-xs font-semibold uppercase tracking-wider">{{ $property->propertyType->name ?? 'Estate' }}</span>
                                <span class="text-gray-300">•</span>
                                <span class="text-gray-400 text-xs">{{ $property->state }}</span>
                            </div>
                            <h3 class="font-display font-bold text-[#0A1628] text-base mb-1.5 group-hover:text-[#C9A84C] transition-colors leading-snug line-clamp-2">
                                {{ $property->name }}
                            </h3>
                            ...
                            <div class="flex items-center justify-between mt-auto pt-4 border-t border-gray-100">
                                <div>
                                    ...
                                    <p class="text-[#C9A84C] font-bold text-lg font-display">{{ $property->formatted_price }}</p>
                                </div>
                                <a href="{{ route('properties.show', $property->slug) }}"
                                   class="bg-[#0A1628] hover:bg-[#C9A84C] text-white hover:text-[#0A1628] text-xs font-semibold px-4 py-2.5 rounded-xl transition-all">
                                    View Estate
                                </a>
                            </div>
                        </div>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 p-16 text-center">
                    ...
                    <h3 class="font-display font-bold text-[#0A1628] text-xl mb-3">No Properties Found</h3>
                    ...
                    <a href="{{ route('properties.index') }}"
                       class="inline-flex items-center gap-2 bg-[#C9A84C] text-[#0A1628] font-bold px-8 py-3 rounded-full hover:bg-[#E8C97A] transition-all">
                        Clear Filters
                    </a>
                </div>
                @endif
