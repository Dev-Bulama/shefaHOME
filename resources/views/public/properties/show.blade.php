...
<nav class="bg-[#1A237E] py-3">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <ol class="flex items-center gap-2 text-sm text-gray-400 flex-wrap">
            ...
            <li><a href="{{ route('properties.index') }}" class="hover:text-[#27AE22] transition-colors">Properties</a></li>
            <li><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg></li>
            <li class="text-[#27AE22] truncate max-w-[200px]">{{ $property->name }}</li>
        </ol>
    </div>
</nav>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 mb-6"
                    ...
                    <div class="relative h-72 sm:h-96 lg:h-[480px] overflow-hidden">
                        ...
                        <div class="absolute inset-0 bg-gradient-to-t from-[#1A237E]/30 via-transparent to-transparent pointer-events-none"></div>
                        ...
                        {{-- GLightbox trigger --}}
                        <a :href="mainImage" class="glightbox absolute bottom-5 right-5 bg-black/50 backdrop-blur-sm text-white p-2.5 rounded-xl hover:bg-[#27AE22] hover:text-[#1A237E] transition-all" data-gallery="property-gallery">
                    ...
                    <div class="p-4 flex gap-3 overflow-x-auto">
                        <template x-for="(img, i) in images" :key="i">
                            <button @click="setImage(img, i)"
                                    :class="activeIndex === i ? 'ring-2 ring-[#27AE22] ring-offset-2' : 'opacity-60 hover:opacity-100'"
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-start gap-4 justify-between mb-6">
                        <div>
                            <div class="flex items-center gap-2 mb-2">
                                <span class="text-[#27AE22] text-sm font-semibold uppercase tracking-wider">{{ $property->propertyType->name ?? 'Estate' }}</span>
                            </div>
                            <h1 class="font-display text-3xl sm:text-4xl font-bold text-[#1A237E] mb-2 leading-tight">
                                {{ $property->name }}
                            </h1>
                        ...
                        <div class="text-right flex-shrink-0">
                            ...
                            <p class="font-display text-3xl font-bold text-[#27AE22]">{{ $property->formatted_price }}</p>
                        </div>
                    </div>
                    ...
                    <div x-data="{ tab: 'overview' }">
                        <div class="flex gap-1 border-b border-gray-200 mb-6 overflow-x-auto">
                            ...
                            <button @click="tab = '{{ $key }}'"
                                    :class="tab === '{{ $key }}' ? 'border-[#27AE22] text-[#27AE22]' : 'border-transparent text-gray-500 hover:text-[#1A237E]'"
                        ...
                        <div x-show="tab === 'overview'" x-transition>
                            ...
                            <div>
                                <h3 class="font-bold text-[#1A237E] mb-4">Estate Features</h3>
                        ...
                        <div x-show="tab === 'plans'" x-transition>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                ...
                                <div class="border-2 {{ $plan->is_recommended ? 'border-[#27AE22]' : 'border-gray-200' }} rounded-2xl p-5 text-center relative">
                                    @if($plan->is_recommended)
                                    <span class="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#27AE22] text-[#1A237E] text-xs font-bold px-3 py-1 rounded-full">Recommended</span>
                                    @endif
                                    <h4 class="font-bold text-[#1A237E] mb-2">{{ $plan->name }}</h4>
                                    <p class="text-2xl font-display font-bold text-[#27AE22] mb-1">{{ $plan->formatted_price }}</p>
                                    ...
                                    @endif
                                    <a href="{{ route('inquiries.store') }}" class="block bg-[#1A237E] hover:bg-[#27AE22] hover:text-[#1A237E] text-white text-sm font-semibold py-2.5 rounded-xl transition-all">Select Plan</a>
                                </div>
                                @endforeach
                            </div>
                            @else
                            <div class="text-center py-10 text-gray-400">
                                ...
                                <a href="#inquiry-form" class="inline-block mt-3 text-[#27AE22] font-semibold hover:underline">Enquire Now</a>
                            </div>
                            @endif
                        </div>
                        ...
                        <div x-show="tab === 'tour'" x-transition>
                            ...
                            <div class="rounded-2xl overflow-hidden aspect-video bg-[#1A237E]">
                ...
                <div class="sticky top-24 space-y-4">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden" id="inquiry-form">
                        ...
                        <div class="p-6"
                            ...
                            <h3 class="font-display font-bold text-[#1A237E] text-lg mb-1">Make an Enquiry</h3>
                            ...
                            <div x-show="!success" class="space-y-3">
                                <input type="text" x-model="name" placeholder="Full Name *" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#27AE22] text-[#1A237E]">
                                <input type="email" x-model="email" placeholder="Email Address *" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#27AE22] text-[#1A237E]">
                                <input type="tel" x-model="phone" placeholder="Phone Number *" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#27AE22] text-[#1A237E]">
                                <textarea x-model="message" rows="3" placeholder="Message" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#27AE22] text-[#1A237E] resize-none"></textarea>
                                <button @click="submit()" :disabled="loading"
                                        class="w-full bg-[#27AE22] hover:bg-[#1D9418] text-[#1A237E] font-bold py-3.5 rounded-xl transition-all hover:scale-[1.02] disabled:opacity-70 flex items-center justify-center gap-2">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', config('shefahomes.whatsapp', '2349122388541')) }}?text={{ urlencode('Hello, I am interested in ' . $property->name . '. Please provide more information.') }}"
                        ...
                        <a href="{{ route('properties.show', $property->slug) }}#inquiry-form"
                           class="flex items-center justify-center gap-2 bg-[#1A237E] hover:bg-[#0D1566] text-white font-semibold py-3.5 rounded-xl transition-all text-sm">
                    ...
                    <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                        <h4 class="font-bold text-[#1A237E] text-sm mb-4 uppercase tracking-wider">Property Details</h4>
                        <div class="space-y-3">
                            @foreach([
                            ...
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span class="font-semibold text-[#1A237E] text-right max-w-[60%]">{{ $val }}</span>
                            </div>
                            @endforeach
                        </div>
                    </div>
        ...
        <div class="mt-16">
            <div class="flex items-center justify-between mb-8">
                <h2 class="font-display text-2xl font-bold text-[#1A237E]">Related <span class="text-[#27AE22]">Properties</span></h2>
                <a href="{{ route('properties.index') }}" class="text-sm text-[#27AE22] hover:underline font-medium">View All</a>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <a href="{{ route('properties.show', $r->slug) }}"
                   class="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg border border-gray-100 transition-all hover:-translate-y-1 flex flex-col">
                    ...
                    <div class="p-5">
                        <p class="text-[#27AE22] text-xs font-semibold uppercase tracking-wider mb-1">{{ $r->propertyType->name ?? 'Estate' }}</p>
                        <h3 class="font-bold text-[#1A237E] mb-1 group-hover:text-[#27AE22] transition-colors">{{ $r->name }}</h3>
                        <p class="text-gray-400 text-xs mb-3">{{ $r->location }}, {{ $r->state }}</p>
                        <p class="text-[#27AE22] font-bold font-display">{{ $r->formatted_price }}</p>
                    </div>
                </a>
                @endforeach
            </div>
