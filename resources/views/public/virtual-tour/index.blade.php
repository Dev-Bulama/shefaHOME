...
<section class="bg-[#1A237E] py-20 relative overflow-hidden">
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <span class="inline-block text-[#27AE22] font-semibold text-sm tracking-widest uppercase mb-3">Immersive Experience</span>
        <h1 class="font-display text-5xl lg:text-6xl font-bold text-white mb-4">
            Virtual <span class="text-[#27AE22]">Tours</span>
        </h1>
...
<section class="py-16 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"
        ...
        <div class="mb-10">
            <h2 class="font-display text-2xl font-bold text-[#1A237E] mb-6">Select an Estate to Tour</h2>
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                <button @click="setTour({{ json_encode(['id' => $tour->id, 'url' => $tour->tour_url, 'name' => $tour->name, 'description' => $tour->description ?? '']) }})"
                        :class="activeTour && activeTour.id === {{ $tour->id }} ? 'ring-2 ring-[#27AE22] ring-offset-2 bg-[#1A237E] text-white' : 'bg-white text-gray-700 hover:border-[#27AE22]/50'"
                    ...
                    <div class="relative h-32 bg-[#1A237E]/10 overflow-hidden">
                        ...
                        @endif
                        <div class="absolute inset-0 bg-gradient-to-t from-[#1A237E]/60 to-transparent"></div>
                        {{-- VR icon --}}
                        <div class="absolute top-3 right-3 w-7 h-7 bg-[#27AE22] rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <svg class="w-4 h-4 text-[#1A237E]" fill="currentColor" viewBox="0 0 24 24"><path d="M20.5 6c-2.61.7-5.67 1-8.5 1s-5.89-.3-8.5-1L3 8c1.86.5 4 .83 6 1v13h2v-6h2v6h2V9c2-.17 4.14-.5 6-1l-.5-2z"/></svg>
                        </div>
                    </div>
                    <div class="p-3">
        ...
        <div x-show="activeTour" class="mb-10">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <div>
                        <h3 class="font-display font-bold text-[#1A237E]" x-text="activeTour?.name"></h3>
                        <p class="text-gray-400 text-sm" x-text="activeTour?.description"></p>
                    </div>
                    <div class="flex items-center gap-2">
                        <span class="inline-flex items-center gap-1.5 bg-[#27AE22]/15 text-[#1D9418] text-xs font-bold px-3 py-1.5 rounded-full">
        ...
        <template x-if="activeTour">
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <div class="flex flex-col sm:flex-row items-start gap-6">
                    <div class="flex-1">
                        <h4 class="font-display text-xl font-bold text-[#1A237E] mb-2" x-text="activeTour.name"></h4>
                    ...
                    <div class="flex-shrink-0 flex gap-3">
                        ...
                        <a href="{{ route('contact') }}"
                           class="border border-[#1A237E] text-[#1A237E] hover:bg-[#1A237E] hover:text-white font-semibold text-sm px-6 py-3 rounded-xl transition-all">
                            Book Site Visit
                        </a>
                    </div>
                </div>
        ...
        <div class="text-center py-24 bg-white rounded-2xl border border-gray-100">
            <div class="w-20 h-20 mx-auto mb-6 bg-[#27AE22]/10 rounded-full flex items-center justify-center">
                <svg class="w-10 h-10 text-[#27AE22]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
            </div>
            <h3 class="font-display text-2xl font-bold text-[#1A237E] mb-3">Virtual Tours Coming Soon</h3>
            ...
            <a href="{{ route('contact') }}" class="bg-[#27AE22] text-[#1A237E] font-bold px-10 py-4 rounded-full hover:bg-[#4ADE80] transition-all">
                Book a Site Visit
            </a>
        </div>
        @endif
...
<section class="py-16 bg-[#1A237E] text-center">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-wrap gap-4 justify-center">
            <a href="{{ route('properties.index') }}" class="bg-[#27AE22] hover:bg-[#4ADE80] text-[#1A237E] font-bold px-8 py-4 rounded-full transition-all hover:scale-105">
                Browse Estates
            </a>
            <a href="{{ route('contact') }}" class="border-2 border-white text-white hover:bg-white hover:text-[#1A237E] font-bold px-8 py-4 rounded-full transition-all">
                Book Site Visit
            </a>
        </div>
    </div>
